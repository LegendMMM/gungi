...
typedef struct HybridPolicyMoveOrder {
    Move move;
    float score;
    int index;
} HybridPolicyMoveOrder;
...
static int compare_policy_move_order_desc(const void *left, const void *right)
{
    const HybridPolicyMoveOrder *a = (const HybridPolicyMoveOrder *)left;
    const HybridPolicyMoveOrder *b = (const HybridPolicyMoveOrder *)right;
...
static int vp_model_black_score(const GungiVPModel *model, const GameState *state, float model_scale)
{
    float value = gungi_vp_value_evaluate(model, state);
    if (state->current_player == GUNGI_PLAYER_WHITE) {
        value = -value;
    }
    return (int)(value * model_scale);
...
int gungi_vp_hybrid_leaf_score(const GungiVPModel *model,
                               const GameState *state,
                               const GungiHybridSearchConfig *config)
{
    const GungiHybridSearchConfig *active = effective_config(config);
    ...
    score = vp_model_black_score(model, state, active->model_scale);
    if (active->mode == GUNGI_HYBRID_EVAL_HYBRID) {
        score += gungi_evaluate_board(state);
    }
...
static void order_vp_moves(const GungiVPModel *model,
                           const GameState *state,
                           Move *moves,
                           int count)
{
    HybridPolicyMoveOrder ordered[GUNGI_MAX_LEGAL_MOVES];
    int i;

    for (i = 0; i < count; ++i) {
        ordered[i].move = moves[i];
        ordered[i].score = gungi_p_score_move(model, state, moves[i]);
        ordered[i].index = i;
    }

    qsort(ordered, (size_t)count, sizeof(ordered[0]), compare_policy_move_order_desc);
...
static int vp_hybrid_minimax(GameState *state,
                             ...
                             const GungiHybridSearchConfig *config)
{
    ...
    if (depth == 0 || state->status != GUNGI_STATUS_ONGOING) {
        return gungi_vp_hybrid_leaf_score(model, state, config);
    }
    ...
    maximizing = state->current_player == GUNGI_PLAYER_BLACK;
    order_vp_moves(model, state, moves, count);
    ...
    if (maximizing) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            gungi_apply_move(&next_state, moves[i]);
            eval = vp_hybrid_minimax(&next_state, model, depth - 1, alpha, beta, config);
            if (eval > best) {
                best = eval;
            }
            if (eval > alpha) {
...
static const GungiVPModel *vp_policy_for_side(const GameState *state,
                                              const GungiVPModel *black_policy_model,
                                              const GungiVPModel *white_policy_model)
{
    if (state != NULL && state->current_player == GUNGI_PLAYER_WHITE && white_policy_model != NULL) {
        return white_policy_model;
    }
    if (black_policy_model != NULL) {
        return black_policy_model;
    }
    return white_policy_model;
}

static int vp_side_policy_hybrid_minimax(GameState *state,
                                         const GungiVPModel *value_model,
                                         const GungiVPModel *black_policy_model,
                                         const GungiVPModel *white_policy_model,
                                         int depth,
                                         int alpha,
...
{
    Move moves[GUNGI_MAX_LEGAL_MOVES];
    const GungiVPModel *policy_model;
    int count;
    int maximizing;
    int i;

    if (depth == 0 || state->status != GUNGI_STATUS_ONGOING) {
        return gungi_vp_hybrid_leaf_score(value_model, state, config);
    }
    ...
    maximizing = state->current_player == GUNGI_PLAYER_BLACK;
    policy_model = vp_policy_for_side(state, black_policy_model, white_policy_model);
    if (policy_model != NULL) {
        order_vp_moves(policy_model, state, moves, count);
    }
    ...
    if (maximizing) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            gungi_apply_move(&next_state, moves[i]);
            eval = vp_side_policy_hybrid_minimax(&next_state,
                                                 value_model,
                                                 black_policy_model,
                                                 white_policy_model,
                                                 depth - 1,
                                 ...
                                 const GungiHybridSearchConfig *config)
{
    ...
    copy = *state;
    return vp_hybrid_minimax(&copy, model, depth, -INT_MAX, INT_MAX, active);
}
...
#pragma omp parallel for
    for (i = 0; i < count; ++i) {
        ...
        gungi_apply_move(&next_state, moves[i]);
        eval = vp_hybrid_minimax(&next_state, model, depth - 1, -INT_MAX, INT_MAX, active);
...
Move gungi_get_vp_hybrid_ai_move(const GameState *state,
                                 ...
                                 const GungiHybridSearchConfig *config)
{
    return gungi_get_vp_hybrid_ai_move_scored(state, model, depth, config, NULL);
}
                                                    ...
                                                    int *out_black_score)
{
    ...
    if (state == NULL || value_model == NULL) {
        return gungi_make_resign(state != NULL ? state->current_player : GUNGI_PLAYER_BLACK);
    }
...
Move gungi_get_vp_side_policy_hybrid_ai_move(const GameState *state,
                                             ...
                                             const GungiHybridSearchConfig *config)
{
    return gungi_get_vp_side_policy_hybrid_ai_move_scored(state,
                                                          value_model,
                                                          black_policy_model,
                                                          white_policy_model,
                                                          depth,
